...
/*EXECICIO 8: OBJETIVO: RETORNAR EM FORMATO DE TABELA INFORMAÇÕES
DE TODOS OS ASSINANTES QUE ESTÃO COM ASSINATURA VENCIDA 
OU JA, NÃO ESTIVER PAGAS E ESTIVER ATRASADA*/

CREATE FUNCTION FUNC_PROJ_EXERCICIO_8()
RETURNS TABLE
RETURN (SELECT A.ASSINATURAID, C.NOME,C.CLIENTEID, R.PRECO, A.DATA_VENCIMENTO,GETDATE() AS [DATA ATUAL]
			FROM PROJ_ASSINATURA A INNER JOIN PROJ_CLIENTE C
			ON A.CLIENTEID = C.CLIENTEID
			INNER JOIN PROJ_REVISTA R
			ON A.REVISTAID = R.REVISTAID
			WHERE SIT_PAGAMENTO = 'N'
			AND DATA_VENCIMENTO < GETDATE())
SELECT * FROM FUNC_PROJ_EXERCICIO_8()

/*EXECICIO 9: OBJETIVO: SE UMA ASSINATURA (RECEBIDA PELO ID) ESTIVER VENCIDA, OU JA, 
NÃO ESTIVER PAGAS E ESTIVER ATRASADA, E A FORMA DE PAGAMENTO FOR 'C' (CRÉDITO) A JUNÇÃO RETORNARÁ 
O VALOR (PRECO DA REVISTA) + JUROS A SER PAGO, 
SE NÃO RETORNARÁ O VALOR SEM O JUROS*/

CREATE PROCEDURE PROC_PROJ_EXERCICIO_9 
@ID_ASS NUMERIC(18),
@VALOR_A_PAGAR NUMERIC(18,2) OUTPUT
AS
BEGIN 
	IF EXISTS(SELECT *
				FROM PROJ_ASSINATURA A INNER JOIN PROJ_REVISTA R
		...
		BEGIN
			...
			PRINT 'ASSINANTE COM ID: ' + CONVERT(varchar(10), @ID_ASS)+ ' TEVE O VALOR AUMENTADO PARA: R$'+CONVERT(varchar(10), @VALOR_A_PAGAR)
			
		END
	ELSE
		BEGIN
			SELECT @VALOR_A_PAGAR = R.PRECO
				FROM PROJ_ASSINATURA A INNER JOIN PROJ_REVISTA R
					ON A.REVISTAID = R.REVISTAID
					INNER JOIN PROJ_PAGAMENTO P
					ON A.FORMA_PAGAMENTOID = P.PAGAMENTOID
					...
					PRINT 'NÃO HÁ JUROS PARA ESTE CLIENTE'
		END
END


/**EXERCICIO 10 - OBJETIVO: INFORMAR O VALOR TOTAL DE TODAS AS VENDAS PARA
UMA ID ESPECIFICA OU (0) PARA TODAS AS REVISTAS, CASO DE ALGUM ERRO O 
USUÁRIO É INFORMADO*/

CREATE PROCEDURE PROC_PROJ_EXERCICIO_10
@ID NUMERIC(18,0)
AS
BEGIN
	IF (@ID = 0)
	BEGIN
		SELECT R.REVISTAID, R.NOME, (R.PRECO) AS [TOTAL DE VENDAS S/ CONSIDERAR JUROS EM PAGAMENTOS]
				FROM PROJ_ASSINATURA A INNER JOIN PROJ_REVISTA R
					ON A.REVISTAID = R.REVISTAID
					INNER JOIN PROJ_PAGAMENTO P
					ON A.FORMA_PAGAMENTOID = P.PAGAMENTOID
					GROUP BY R.REVISTAID, R.NOME,R.PRECO
	END
	ELSE IF (@ID > 0)
	BEGIN
		SELECT R.REVISTAID, R.NOME, (R.PRECO) AS [TOTAL DE VENDAS S/ CONSIDERAR JUROS EM PAGAMENTOS]
				FROM PROJ_ASSINATURA A INNER JOIN PROJ_REVISTA R
					...
					INNER JOIN PROJ_PAGAMENTO P
					ON A.FORMA_PAGAMENTOID = P.PAGAMENTOID
					WHERE R.REVISTAID = @ID
					GROUP BY R.REVISTAID, R.NOME,R.PRECO
	END
	
	ELSE
		RAISERROR('INFORME OUTRO ID, ESTE ID É INEXISTENTE',13,1)
	
END

/*EXERCICIO 11 OBJETIVO: TRIGGER CRIADA PARA SER ACIONADA QUANDO HOUVER REGITROS 
INSERIDOS, DELETADOS OU ALTERADOS NA TABELA PROJ_ASSINATURA*/

CREATE TRIGGER TRG_PROJ_EXERCICIO_11
ON PROJ_ASSINATURA AFTER INSERT,UPDATE,DELETE
AS
BEGIN
	DECLARE @ID_N NUMERIC(18,0)
	DECLARE @ID_A NUMERIC(18,0)
	SET @ID_N = (SELECT ASSINATURAID FROM inserted)
	SET @ID_A = (SELECT ASSINATURAID FROM deleted)

	IF EXISTS (SELECT * FROM inserted)
		INSERT INTO PROJ_LOGS (DESCRICAO,NOME,TABELAALTERADA,DATAALTERACAO,DATAINSERCAO) VALUES 
		('INSERIDO ASSINATURA DE ID: '+CONVERT(VARCHAR(5),@ID_N),CONVERT(VARCHAR(20),SUSER_SNAME()),'PROJ_ASSINATURA',NULL,GETDATE())
	ELSE IF EXISTS (SELECT * FROM deleted)
		INSERT INTO PROJ_LOGS (DESCRICAO,NOME,TABELAALTERADA,DATAALTERACAO,DATAINSERCAO) VALUES 
		('DELETANDO ASSINATURA DE ID: '+CONVERT(VARCHAR(5),@ID_A),CONVERT(VARCHAR(20),SUSER_SNAME()),'PROJ_ASSINATURA',GETDATE(),NULL)
	ELSE
		INSERT INTO PROJ_LOGS (DESCRICAO,NOME,TABELAALTERADA,DATAALTERACAO,DATAINSERCAO) VALUES 
		('ATUALIZADO ASSINATURA DE ID: '+CONVERT(VARCHAR(5),@ID_N),CONVERT(VARCHAR(20),SUSER_SNAME()),'PROJ_ASSINATURA',GETDATE(),NULL)

END


/*EXERCICIO 12    OBJETIVO: PERMITIR A INTEGRIDADE DO BANCO E TAMBÉM A REGRA DE NEGÓCIO
NÃO PERMITIR QUE CLIENTES MENORES DE IDADE ASSINEM REVISTAS COM FAIXA ETARIA PARA MAIORES
DE IDADE*/

CREATE TRIGGER TRG_PROJ_EXERCICIO_12
ON PROJ_ASSINATURA INSTEAD OF INSERT
AS
BEGIN
	DECLARE @IDADE NUMERIC(18,0)
	DECLARE @FAIXA NUMERIC(18,0)

	SELECT @IDADE = (YEAR(GETDATE()) - YEAR(C.DATA_NASCIMENTO))
	FROM inserted D INNER JOIN PROJ_CLIENTE C
		ON D.CLIENTEID = C.CLIENTEID

	SELECT @FAIXA = R.FAIXA_ETARIA
		FROM inserted D INNER JOIN PROJ_REVISTA R
			ON D.REVISTAID = R.REVISTAID

	IF (@IDADE < 18 AND @FAIXA = 18)
		BEGIN			
			RAISERROR('CLIENTE MENOR DE IDADE NÃO PODE ASSINAR REVISTA COM ESTA FAIXA ETARIA',13,1,1)
		END
	ELSE IF (@IDADE >=18)
		BEGIN
			INSERT INTO PROJ_ASSINATURA SELECT * FROM INSERTED
			PRINT 'CADASTRO EFETUADO COM SUCESSO'
		END
	ELSE IF(@IDADE < 18 AND @FAIXA <> 18)
		BEGIN
			INSERT INTO PROJ_ASSINATURA SELECT * FROM INSERTED
			PRINT 'CADASTRO EFETUADO COM SUCESSO. CLIENTE MENOR DE IDADE E FAIXA MENOR QUE 18 ANOS'
		END
	ELSE
		RAISERROR('FALHA INESPERADA',13,1,1)
END

/*EXERCICIO 13 OBJETIVO: CURSOR CRIADO SOMENTE PARA PERSONALIZAR A MENSAGEM DE DATA DE CADASTRO
DO CLIENTE OU DA CLIENTE PARA UM POSSÍVEL ENVENTO*/

CREATE PROCEDURE PROC_PROJ_EXERCICIO_13
AS
BEGIN
	...
	FOR
	SELECT NOME,DATA_CADASTRO,SEXO FROM PROJ_CLIENTE ORDER BY NOME
	OPEN curCliente

	FETCH NEXT FROM curCliente INTO @NOME, @DATA,@SEXO
	WHILE @@FETCH_STATUS = 0
		BEGIN
			IF(@SEXO = 'M')
				PRINT 'O CLIENTE '+ @NOME + ' ESTÁ CADASTRADO DESTE ' + CONVERT(VARCHAR(50),@DATA) + ' '
			IF(@SEXO = 'F')
				PRINT 'A CLIENTE '+ @NOME + ' ESTÁ CADASTRADA DESTE ' + CONVERT(VARCHAR(50),@DATA) + ' '
